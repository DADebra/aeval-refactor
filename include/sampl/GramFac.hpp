  ...
  class GRAMfactory
  {
    ...
    bool parseGramFile(string gram_file, string inv_fname)
    {
      ...
      if (gram_file.empty())
        return true;
      gramgiven = true;
      ostringstream gramfile;
      gramfile << ifstream(gram_file).rdbuf();
      return parseGramString(gramfile.str(), inv_fname);
    }

    bool parseGramString(string grammar, string inv_fname)
    {
      if (grammar.empty())
        return true;
      gramgiven = true;
